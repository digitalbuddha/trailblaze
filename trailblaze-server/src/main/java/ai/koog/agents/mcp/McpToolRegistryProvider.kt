...
public object McpToolRegistryProvider {
   ...
   */
  public fun defaultStdioTransport(process: Process): StdioClientTransport = StdioClientTransport(
    ...
    return ToolRegistry {
      sdkTools.forEach { sdkTool ->
        try {
          ...
          println("Failed to parse descriptor parameters for tool: ${sdkTool.name} ${e.printStackTrace()}")
        }
      }
    }
  }
